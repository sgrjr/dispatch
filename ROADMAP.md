...
- [ ] **T3 — per-approval TTL in the Agent Sessions UI.** `AgentSessionService::approve()` already accepts a per-session `$ttl` the Livewire approve action never passes — surface it so the approver right-sizes the window per commission (short for experiments, long for overnight runs). The better long-term home for the T2 decision; the 3h default then becomes just the pre-selected option.
...
**Status: T1/T2 COMPLETE (2026-07-20)** — rides the pending **v0.6.0** release. Docs: README "Estimated human touch-time" block + TTL snippet, UPGRADING v0.6.0 ×2 bullets. Open: T3 (UI seam), T4 (calibration, operator-paced).
