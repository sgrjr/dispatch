...
- [ ] **M1 — codeless keyed upsert on `dispatch:import` (the one blocker; was G1 + G5-truncation).** `DispatchImport` skips any row without a `code` (`DispatchImport.php:100-104`), so a fresh md has nothing to key on and re-imports duplicate. **Fill:** when `code` is absent, upsert by a stable `key` (host = `sha1(file|first-line)`) persisted as `dedupe_key`, minting the code via the paths the batch verb already uses (`DispatchTaskService::firstOrCreateByKey()` `:269` + `Task::createWithCode()` `:253`) — the mechanism exists; this is wiring `import` to reuse it. **Fold in here too:** the import UPDATE path (`$task->fill($payload)`, `DispatchImport.php:131`) bypasses `create()`'s 255-char title truncation (`DispatchTaskService.php:42`), so a re-import of a long-titled row can overflow — truncate on the update path as well.
- [ ] **M2 — `--quiet` suppress-notify on the bulk backfill paths (import **and** batch; was G2, widened).** Both `dispatch:import` and `dispatch:batch add` route through `DispatchTaskService::create()`, which unconditionally calls `DispatchNotifier::taskCreated()` (`DispatchTaskService.php:70`). A historical backfill would (a) email a spurious "request received" for every row whose submitter email resolves, and (b) if the host wired the reactive `EventNotifier`, fire the orchestration hook once per imported task. `MailNotifier` no-ops on a null submitter (covers most historical rows) but that's incidental, not a contract. **Fill:** a `--quiet` path that suppresses notifications + reactive automation for a bulk run — cross-cutting, not import-only.
- [ ] **M3 — publish the frozen import shape via `dispatch:schema` (was G3).** The import JSON shape is defined only implicitly by `DispatchImport`/`DispatchExport`, so a host writes the md→JSON translator blind. **Fill:** add an `import` key to the frozen `dispatch:schema` contract (mirror how `batch` was folded in) so translators target a documented, versioned shape instead of reverse-engineering the reader.
- [ ] **M4 — one migration guide + `dispatch-batch-migrate` skill upgrade (doc-only; folds G6/G7/G8 + provenance).** Every host otherwise re-derives the same conventions — ship them once:
  - **Flatten convention (was G6 — decided doc, NOT schema):** Task is deliberately flat (no `parent_id`; a locked choice). Document folding md `  - [x]` sub-items into the parent description, or emitting them as separate tasks linked by a `parent:<code>` label / `context.parent`. **Not adding a real parent/child relation.**
  - **Vocabulary map (was G7):** recommend kind→nearest `type` while **preserving the original as `kind:*` / `state:*` / `size:*` + domain labels**; note `workflow.types` is config-extensible for a host wanting richer native types.
  - **Input contract + garbage guard (was G8):** document that only task-bullets (`- [ ]`/`- [x]`) become tasks (headings/dates ride along as labels/context), and keep the existing skip-and-count guard (`tasks_skipped`) so a messy md can't silently corrupt the board.
  - **Provenance convention:** bake in `label source:todo-md`, `context.source = {file, line, imported_at}`, `key = sha1(file|first-line)` (the last feeds M1).
